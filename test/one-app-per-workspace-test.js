...
assert(serviceQml.indexOf('closewindow') !== -1, 'keeps the delayed close fallback')
assert(earlyHook.indexOf('window.open_early') !== -1, 'registers the pre-layout window event')
assert(earlyHook.indexOf('window.close') !== -1, 'registers the pre-close window event')
assert(earlyHook.indexOf('workspace = "emptynm"') !== -1, 'moves early windows to an empty workspace')
assert(earlyHook.indexOf('hl.get_workspaces') !== -1, 'checks occupied workspaces before closing')
assert(earlyHook.indexOf('hl.dsp.focus') !== -1, 'focuses the destination before closing')
